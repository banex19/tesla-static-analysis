...
std::pair<Value *, std::string> GetValueName(DbgDeclareInst *v)
{
    ...
    auto var = v->getVariable();

    if (var)
    {
        return std::make_pair(varValue, var->getName());
    }
    ...
    Module &Mod, IRBuilder<> &Builder)
{
    ...
    for (auto &I : EntryBlock)
    {
        ...
        if (Inst)
        {
            ...
            if (valName.first != nullptr)
            {
                //  llvm::errs() << "Value (DbgDeclare): " << *valName.first << "\n";
                ValuesInScope[valName.second] = Builder.CreateLoad(valName.first);
            }
        }
        else
        {
            ...
            if (Inst)
            {
                ...
                if (valName.first != nullptr)
                {
                    //  llvm::errs() << "Value (DbgValue): " << *valName.first << "\n";
                    if (dyn_cast<AllocaInst>(valName.first) != nullptr)
                        ValuesInScope[valName.second] = Builder.CreateLoad(valName.first);
                    else
                        ValuesInScope[valName.second] = valName.first;
                }
            }
        }
    }
